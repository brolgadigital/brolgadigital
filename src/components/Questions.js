...
export default function Questions(props, i) {
    ...
    const toggleFAQ = (e) => {
        ...
        else if (trigger.classList.contains('openQ') || trigger.parentElement.classList.contains('openQ')) {
            closeDrawer(open);
            console.log('clicked');
        }
    ...
    return (
        <div className='question' onClick={toggleFAQ} onKeyDown={toggleFAQ} role='button' tabIndex={i}>
            <h4>{props.q}</h4>
            <p>{props.a}</p>
        </div>
    )
